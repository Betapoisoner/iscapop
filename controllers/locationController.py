...
class LocationController(http.Controller):
    ...
    def getLocations(self, locationId=None, **kw):
        try:
            ...
            locations = http.request.env['iscapop.location_model'].search_read(
                domain, ['id', 'name', 'description', 'loc_type', 'stock_full', 'details_ids'])
    ...
    def addLocation(self,):
        try:
            ...
            result = http.request.env['iscapop.location_model'].create(
                location)
            data = {
                "status": 201,
                "data": result.id
