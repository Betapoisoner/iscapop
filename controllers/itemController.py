...
class ItemController(http.Controller):
    ...
    def getItems(self, itemId=None, **kw):
        try:
            ...
            items = http.request.env['iscapop.item_model'].search_read(
                domain, ['id', 'name', 'description', 'stock_full', 'details_ids', 'category_id'])
            for item in items:
                details_list = []
                for detail_id in item.get('details_ids', []):
                    detail = http.request.env['iscapop.item_details_model'].search_read([("id", "=", detail_id)], [
                                                                                        'id', 'condition', 'state', 'reserved', 'location_id', 'donation_id', 'stock'])
                    if detail:
                ...
                # Assign the list of details back to the item
                item['details_ids'] = details_list
