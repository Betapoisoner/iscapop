...
class CategoryController(http.Controller):
    ...
    def getCategories(self, categoryId=None, **kw):
        try:
            ...
            categories = http.request.env['iscapop.category_model'].search_read(
                domain, ['id', 'name', 'complete_name', 'description', 'category_son_ids', 'item_ids'])
            for category in categories:
                categorys_list = []
                for category_id in category['category_son_ids']:
                    category = http.request.env['iscapop.category_model'].search_read(
                        [("id", "=", category_id)], ['id', 'name', 'description', 'stock_full', 'details_ids'])
                    if category:
                        category_data = category[0]
                        # Append the modified detail dictionary
                        categorys_list.append(category_data)

                # Assign the list of details back to the category
                category['category_ids'] = categorys_list
    ...
    def addCategory(self,):
        try:
            ...
            result = http.request.env['iscapop.category_model'].create(
                category)
            data = {
                "status": 201,
                "data": result.id
    ...
    def updLocation(self, categoryId=None, **kw):
        try:
            ...
            if uid == None:
                ...
                return json_data
            domain = [("id", "=", categoryId), ("create_uid", "=", uid)]
            category = http.request.env['iscapop.category_model'].search(
                domain)
            if category:
