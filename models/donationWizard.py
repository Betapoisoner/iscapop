...
class DonationWizard(models.TransientModel):
    ...
    @api.depends("item_id", "condition", "item_option")
    def _compute_available_stock(self):
        for record in self:
            if record.item_option == "existing" and record.item_id and record.condition:
    ...
    def action_donate(self):
        ...
        if self.item_option == "new":
            if not self.new_item_name:
                raise ValidationError(_("Please enter a name for the new item."))
            # Create new item
            ...
            # Create new item detail in donating state
            ...
            message = _("Successfully created and donated %s %s items!") % (
                self.quantity,
                self.condition,
            )
        else:
            if self.available_stock < self.quantity:
                raise ValidationError(
                    _("Not enough available items for this condition!")
                )
