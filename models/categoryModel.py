...
class categoryModel(models.Model):
    ...
    complete_name = fields.Char(string="Complete Name", compute="_compute_parent")
        ...
        for category in self:
            if category.category_parent_id:
                category.complete_name = "%s/%s" % (
                    category.category_parent_id.complete_name,
                    category.name,
                )
            else:
    ...
    def open_subcategories(self):
        return {
            ...
            "name": "Subcategories",
            "res_model": "iscapop.category_model",
            "domain": [("category_parent_id", "=", self.id)],
            "view_mode": "tree,form",
            "target": "current",
        }
